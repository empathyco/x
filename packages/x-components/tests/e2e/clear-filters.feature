Feature: Clear selected filters
  ...
  Scenario Outline: 3. Remove single filter
    ...
    Then  filters "<filter>" are shown in the selected filters list
    When  filter number <filter> is clicked in facet "<facetName2>"
    Then  filters "<filter>, <filter>" are shown in the selected filters list
    When  filter number <filter> is clicked in facet "<facetName3>"
    Then  filters "<filter>, <filter>, <filter>" are shown in the selected filters list
