...
export function xComponentMixin(
  ...
  return {
    ...
    created(): void {
      /* Iterate through config props, creating a watcher for each one of them that will try to call
       the setConfig component method or if that method does not exist, the setConfig mutation of the module */
