<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props, { attrs, slots }) {
       ...
       * @internal
       */
      let oldPositions!: WeakMap<Element, Bounds>;
      /**
       ...
       * @internal
       */
      let newPositions!: WeakMap<Element, Bounds>;
      /**
       ...
       * @internal
       */
      let pendingCleanupMoveCallbacks!: WeakMap<Element, () => void>;
      /**
       ...
       * @internal
       */
      let staggerCounter!: number;
      /**
       * The bounds of the container rendered using the `tag` prop. This is used to calculate the
       * relative positions of each leaving child, which are then applied with the position
       * absolute.
       *
       * @internal
       */
      ...
      oldChildren = [];
      newChildren = [];
      oldPositions = new WeakMap();
      newPositions = new WeakMap();
      pendingCleanupMoveCallbacks = new WeakMap<Element, () => void>();
      staggerCounter = 0;
       ...
       * @internal
       */
      const addRestorePositionHook = (transitionHook: TransitionHook = noOp): TransitionHook => {
        return element => {
       ...
       * @internal
       */
      const addClearStaggeringCall = (transitionHook: TransitionHook = noOp): TransitionHook => {
        return element => {
       ...
       * virtual node's API.
       *
       * The `beforeLeave` hook is extended to also restore the previous position of the element
      ...
      const transitionData = computed((): TransitionData => {
        const transitionData: TransitionData = { ...props, ...attrs };
       ...
       *
       * @param vNode - The VNode to check if it is a valid transition node, containing
       * a `tag` and a `key` property.
       * @returns True when the vNode contains a non empty key and a non empty tag. False otherwise.
       * @internal
       */
      const isTransitionValidVNode = (vNode: VNode): vNode is TransitionVNode => {
       ...
       * @internal
       */
      const addTransitionData = (vNode: TransitionVNode): void => {
        if (!vNode.data) {
          vNode.data = {};
        }
       ...
       * @internal
       */
      const createRelativeBounds = (vNode: TransitionVNode): Bounds => {
       ...
       * @internal
       */
      const recordOldPosition = (vNode: TransitionVNode): void => {
       ...
       * @internal
       */
      const recordNewPosition = (vNode: TransitionVNode): void => {
       ...
       * @internal
       */
      const syncOldNodes = (vNode: TransitionVNode): void => {
       ...
       * @internal
       */
      const getNodesByTransitionType = (): TransitionTypeNodes => {
       ...
       * @internal
       */
      const getNextTransitionDelay = (): string => {
       ...
       * @internal
       */
      const applyStagger = (vNode: TransitionVNode): void => {
       ...
       *
       * @remarks This is done to avoid letting the user click elements that
       * are performing the moving animation to leave the DOM but are still rendered.
       *
       * @param vNode - The virtual node to disable listening to click events.
       * @internal
       */
      const disableClickingEvents = (vNode: TransitionVNode): void => {
       ...
       * @internal
       */
      const applyTranslation = (vNode: TransitionVNode): boolean => {
      ...
      /**
       * Generates a function to start the moving animations to each node that it needs
       * them with the provided stagger.
       *
      ...
      const render = (): VNode => {
        ...
        newChildren = (slots.default!() ?? []).filter(isTransitionValidVNode);
        ...
        return h(props.tag, { staticClass: 'x-staggering-transition-group' }, newChildren);
      };
