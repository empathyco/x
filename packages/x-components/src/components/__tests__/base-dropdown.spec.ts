...
function renderDropdown({
  ...
  const clickToggleButton = async () => getDropdownToggle().trigger('click')
  const clickNthItem = async (nth: number) => getListItems().at(nth)?.trigger('click')
  const pressKeyFromToggle = async (key: Key) => getDropdownToggle().trigger(`keydown`, { key })
  const pressKeyFromFocusedItem = async (key: Key) =>
  ...
  async function chainTypeChars(search: string, previousCharPromise: Promise<void>): Promise<void> {
    ...
    return charToType
      ? chainTypeChars(
          pendingChars,
          previousCharPromise.then(async () =>
