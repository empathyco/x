 ...
 * @param options.urlParams - urlParams option.
...
describe('testing UrlHandler component', () => {
  ...
  it('changes the URL params when `PushableUrlStateUpdated` is emitted', async () => {
    const { getCurrentUrlParams } = renderUrlHandler({
      template: '<UrlHandler store="store" />',
    })

    await XPlugin.bus.emit('PushableUrlStateUpdated', {
      ...initialUrlState,
      query: 'lego',
      page: 2,
      store: '111',
  ...
  it('changes the URL params when `ReplaceableUrlStateUpdated` is emitted', async () => {
    const { getCurrentUrlParams } = renderUrlHandler({
      template: '<UrlHandler store="store" />',
    })

    await XPlugin.bus.emit('ReplaceableUrlStateUpdated', {
      ...initialUrlState,
      query: 'lego',
      page: 2,
      store: '111',
  ...
  it('normalizes + characters into %20 for spaces when updating the url', async () => {
    renderUrlHandler({
      template: '<UrlHandler store="store" />',
    })
  ...
  it('ignores all parameters if query is not provided', async () => {
    renderUrlHandler()
