...
describe('testing wires operators', () => {
  ...
  describe('testing filtering operators', () => {
    it(`${filter.name} only executes a wire when the condition is true`, () => {
    ...
    describe('truthy and falsy filter operators', () => {
      const falsyValues = [false, '', 0, undefined, null, Number.NaN]
      const truthyValues = [true, 'a5 kobe', -1, {}, [], () => {}]
      it(`${filterFalsyPayload.name} avoids executing the wire when the payload is falsy`, () => {
      ...
      it(`${filterTruthyPayload.name} avoids executing the wire when the payload is truthy`, () => {
