...
describe('platformAdapter tests', () => {
  ...
  it('should call the next queries endpoint', async () => {
    ...
    const response = await platformAdapter.nextQueries({
      query: 'makeup',
      extraParams: {
        instance: 'empathy',
        env: 'staging',
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      // eslint-disable-next-line max-len
      'https://api.staging.empathy.co/nextqueries/empathy?query=makeup&instance=empathy&env=staging&lang=en&device=mobile&scope=mobile',
      { signal: expect.anything() }
    );
  ...
  it('should call the related tags endpoint', async () => {
    ...
    const response = await platformAdapter.relatedTags({
      query: 'jeans',
      extraParams: {
        instance: 'empathy',
        env: 'staging',
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      // eslint-disable-next-line max-len
      'https://api.staging.empathy.co/relatedtags/empathy?query=jeans&instance=empathy&env=staging&lang=en&device=mobile&scope=mobile',
      { signal: expect.anything() }
    );
