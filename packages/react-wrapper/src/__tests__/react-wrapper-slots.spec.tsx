...
describe('Testing support of slots in the react-wrapper', () => {
  ...
  it('updates the scoped slots when the content changes', async () => {
    const reactComponent = ReactDOM.render(<ReactScopedSlotStub/>,
      root) as any as ReactScopedSlotStub;

    await Vue.nextTick();
    expect(root.innerHTML)
      .toEqual(`<section><span>Vue says: Hello</span>${ vueComment }</section>`);
  ...
  it('renders a default and scoped slot in the same component', async () => {
    ...
    ReactDOM.render(<ReactWrapper component={ VueSlots } message={ messageProp } slots={ slots }/>,
      root);
